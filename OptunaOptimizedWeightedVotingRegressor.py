...
class OptunaOptimizedWeightedVotingRegressor:
    ...
    def plot_predictions_vs_actual(self, X_train, X_test, y_train, y_test):
        ...
        plt.scatter(y_test, y_test_pred, color='#D4BE83', s=100, marker='s', alpha=1,
                    label=f'Test List')
        # 放大加粗x轴标签
        plt.xlabel('True Ultimate Bearing Capacity (kN)', fontsize=16, fontweight='bold', fontfamily='Times New Roman')
        # 放大加粗y轴标签
        plt.ylabel('Predicted Ultimate Bearing Capacity (kN)', fontsize=16, fontweight='bold', fontfamily='Times New Roman')
        # 放大加粗标题
        plt.title("Actual vs Predicted Values", fontsize=18, fontweight='bold', fontfamily='Times New Roman')
        # ========== 新增：加粗图表边线、坐标轴轴线和刻度线 ==========
        ax = plt.gca()
        # 加粗坐标轴轴线（x轴和y轴）
        ax.spines['left'].set_linewidth(3.0)
        ...
        plt.text(50, 2450, text_box, fontsize=16, fontweight='bold', fontfamily='Times New Roman',
